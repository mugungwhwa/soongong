...
export function QuestCard({ quest, index }: { quest: Quest; index: number }) {
  return (
    <Card className="px-3 py-2.5 shadow-[var(--shadow-card)] border-[var(--color-border-default)]">
      <div className="flex items-center gap-2">
        <div className="text-xl text-[var(--color-mint-700)] font-bold shrink-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-sm font-semibold text-[var(--color-text-strong)] truncate">
            {quest.topic}
          </div>
          <div className="text-xs text-[var(--color-text-muted)] mt-0.5">
            형식: <b className="text-[var(--color-text-default)]">{quest.questFormat}</b>
            <span className="ml-2 text-[var(--color-mint-700)] font-semibold">+{quest.rewardXp} XP</span>
          </div>
        </div>
        <Link href={ROUTES.play(quest.questId)} className="shrink-0">
          <Button
