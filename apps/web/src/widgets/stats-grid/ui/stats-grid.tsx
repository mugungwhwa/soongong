...
// 카드 치수·등높이는 공용 StatCard + tokens.css --stat-card-* 가 단일 통제(SOO-143).
function StatLink({
  href,
  label,
  children,
  ...
  return (
    <Link
      ...
      className="block h-full rounded-[var(--stat-card-radius)] transition-transform hover:-translate-y-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--color-mint-500)] focus-visible:ring-offset-2"
    >
      {children}
    </Link>
  );
...
export function StatsGrid() {
  ...
  return (
    <div className="grid grid-cols-2 gap-[var(--stat-card-gap)] sm:grid-cols-4 items-stretch">
