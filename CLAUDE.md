...
## 머지 게이트 (2026-06-10 강제 전환 → 2026-07-01 게이트 전환 — 위반 시 머지 불가)

1. main 머지는 PR로만. 기능/수정 코드 PR은 **`agent/<role>/<id>` 브랜치 출신만** 머지한다.
2. 하드 게이트: **아키텍처 린트(FSD layer boundaries) 필수 통과**. 프로세스 게이트: **Tech Lead 구조 리뷰(에이전트) PR 코멘트 필수**. 코드래빗은 보조(옵션)로 강등.
...
4. Tech Lead의 역할 = 아키텍처 정합성 리뷰. 결과는 반드시 **PR 코멘트로** 남긴다
   (커밋 본문 서술 무효). 코드래빗(보조·옵션) ≠ 하드 게이트 — Tech Lead 구조 리뷰 = 프로세스 게이트.
5. **자동 머지 ON (2026-06-20, Mike 무중단 결정).** 위 룰1~4를 모두 충족한 PR
   — `agent/<role>/<id>` 브랜치 + **아키텍처 린트(FSD layer boundaries) 통과** + 필수 체크 green + (코드 PR은 Tech Lead 아키텍처 리뷰 통과)
   — 은 자동 머지된다. 잠금(룰1~4·PR-only·아키텍처 린트 필수·Tech Lead 구조 리뷰)은 그대로이고 **자동화 트리거만 켜진다.**
   **force/admin 머지·실패 체크 우회는 계속 금지(차터 룰7).** 게이트 미충족 PR은 자동 머지되지 않고 대기한다.

> **[2026-07-01 게이트 전환] 트레이드오프:** GitHub 강제 승인 도장(코드래빗 `required_approving_review_count=1`) 제거 → 하드 게이트는 아키텍처 린트(FSD layer boundaries)로 단일화, 리뷰 품질은 에이전트 프로세스(Tech Lead 구조 리뷰)에 의존. **되돌리기 가능**: 예산 여유 시 코드래빗 플랜 결제 후 `required_approving_review_count` 1로 복원하면 강제 승인 도장 재활성화. (Mike 결정 2026-07-01, SOO-148)
