...
인벤토리 §4.5 인용. **자산 배경이 앱 토큰과 정확히 일치하지 않으면 모든 슬롯에서 사각형이 미세하게 노출**됨 (v0.1 측정값: 자산 `#F7F4F2` vs 앱 토큰 `#F8FBF7` → G −7, B −4). 따라서 **전 슬롯 투명화 권장**:
...
→ MVP 1차 기준 **전 슬롯 투명화 1회**. GPT-4o prompt에서 *"투명 배경 또는 정확히 `#F8FBF7` 단색 배경"*을 명시하면 후처리 부담 감소.
...
| **v0.1-alpha** 자동 투명화 | ✅ 부분 완료 | 2026-05-18 | `mascot-v0.1-alpha/main-alpha.png` (완벽, production 가능) + `repeat_normal-alpha.png` (belly 아래 1-2% 잔재, reference 용도만) |
...
| **v1.1** | **2026-05-18** | **§7 매트릭스 "❌ 불필요" 행을 "권장"으로 강등 (자산 #F7F4F2 vs 앱 #F8FBF7 미세 색차 측정값 근거). §8 진행 현황에 v0.1-alpha 자동 투명화 마일스톤 추가 (main 완벽 + repeat_normal 부분 잔재). v0.2 prompt 가이드에 투명 배경 명시 필수 룰 박음.** |
