...
### 5-4. 마스코트 노출 밀도 — "1 뷰포트 = 순공이 1개" (SOO-157)

한 화면을 스크롤할 때 **동시에 보이는 순공이는 히어로 1개로 한정**한다. 인사·축하·빈 상태 등 그 화면의 핵심 모먼트를 대표하는 순공이만 남기고, 보조 카드·리스트 아이템·헤더 옆 장식으로 반복되는 순공이는 **제거**한다(아이콘 치환이 아니라 제거 — 홈 SOO-144(#160) 선례: 인사 블록 순공이를 지우고 CameraHero 히어로만 유지).

- **판정 기준**: 실제 렌더 기준 동시 노출 수 ≤ 1. 조건부 배타 렌더(예: 대기 상태 vs 진행 상태에서 각각 다른 순공이)는 동시에 뜨지 않으므로 예외.
- **히어로 선정**: 그 화면에서 순공이가 *화자/기능*으로 작동하는 위치(오늘 일지 한 줄, 결과 축하, 빈 상태 안내)를 히어로로 유지한다. 단순 헤더 장식은 제거 대상.
- **위젯 합산 주의**: 뷰가 마스코트를 포함한 위젯(예: `ReviewCalendar`)을 렌더하면 뷰 헤더 마스코트와 위젯 마스코트가 합산되어 2개가 된다. 이때 뷰 헤더 쪽을 제거하고 위젯 히어로를 남긴다.
- **근거**: 이 규칙은 "마스코트 전 화면 노출=차용" 결정을 위반하지 않는다(화면당 존재는 유지). 다만 **한 뷰포트 내 반복**이 시각 위계를 흐리고 캐릭터 피로를 유발하므로 금지한다(Mike 2026-07-02, 홈 순공이 3연속 스크린샷 계기).
...
| v2.3 | 2026-07-02 | **§5-4 마스코트 노출 밀도 규칙 신설** — "1 뷰포트 = 순공이 최대 1개". 보조 위치 반복 마스코트는 아이콘 치환이 아니라 제거(홈 SOO-144 선례), 조건부 배타 렌더·위젯 합산 판정 기준 명문화. 홈 외 화면(diary·calendar) 헤더 장식 마스코트 제거 반영 (SOO-157, Mike 홈 3연속 계기). |
