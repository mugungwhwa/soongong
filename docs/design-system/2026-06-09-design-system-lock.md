# 순공대장 디자인 시스템 잠금 v1.0

> **SSoT 출처**: SOO-19 v3 커밋 `110076f` (2026-06-09 확정).  
> **Mike 방향 확정값**: 색 = 바다(Ocean), 로고 = A안, 레이아웃 = v3.  
...
| 토큰명 | Hex | CSS 변수 | 용도 |
|---|---|---|---|
| o50 | `#E8F8FB` | `--m50` | 호버 배경 / 태그·배지 배경 |
| o100 | `#C2EDF5` | `--m100` | Active 배경 (연한 강조) / 게임바 fill 배경 |
| o300 | `#7DD8EA` | `--m300` | XP 게이지 fill / 보조 아이콘 / 보조 강조 |
| o500 | `#2AB8D0` | `--m500` | 메인 브랜드 기본색 (스킬바 fill 등) |
| o700 | `#1A8FAD` | `--m700` | 메인 버튼 / Active 아이콘 · 텍스트 / Active 상태 |
| o900 | `#0E5C82` | `--m900` | 강한 강조 / GameBar 배경 / 로고 기본색 / CTA 버튼 |
...
| bg-sun | `#E2F3F8` | `--bg-sun` | **비텍스트 accent 전용** — 진행바 트랙 / 아이콘 칩 / journey 데코·그라데이션. ※ 인용·지문·원칙 등 **텍스트 블록은 흰색(surface) 기본** (오션 틴트 0, SOO-43 / Mike "블록은 흰색으로"). 텍스트 블록 구분은 hairline border 로. |
...
| 역할 | 시작(gs) | 종료(ge) |
|---|---|---|
| 페이지 바깥 그라데이션 | `#1A8FAD` (o700) | `#0E5C82` (o900) |
...
|---|---|---|
| 라이트 (bg-el) | `#0E5C82` (o900) | `#4A7585` (tm), font-size:9px |
...
- Active nav 항목: `background:#C2EDF5(o100); color:#0E5C82(o900); font-weight:700`
- Hover nav 항목: `background:#E8F8FB(o50); color:#1A8FAD(o700)` 150ms ease
- 퀘스트 row 호버: `border-color:#7DD8EA(o300); background:#E8F8FB(o50)`
