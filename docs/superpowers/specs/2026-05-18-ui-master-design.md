...
### 4.13 상시 누끼 런북 (임의 캐릭터 배경 투명화) ⭐

> **목적**: Mike가 던지는 **임의 캐릭터 이미지**(마스코트 순공이 뿐 아니라 보조/휴먼 캐릭터 포함)를 배경 완전 투명 PNG로 변환하는 **상시 역량**. 1회성이 아니라 반복 절차다. (SOO-79, Mike 확정 2026-06-19)
>
> ⚠️ **값 중복 금지**: 해상도·네이밍·배경색·품질 체크는 §4.5 / §4.6 / §4.7 / §4.8을 **참조**한다. 여기 복붙하지 않는다(두 번째 SSoT 방지).

**트리거** (Mike → Claude 자동 본 런북 read): 캐릭터 이미지 첨부 + "투명/누끼/배경 제거/흰배경 없이/캐릭터 투입". `soongong-design` 스킬이 동일 트리거로 본 §4.13을 가리킨다.

**절차 6단계:**
1. **입력 확보** — 원본(배경 미제거) PNG/JPG/WebP. 휘발성 폴더(예: `~/Downloads`)면 **먼저 repo로 복사**해 불변 스냅샷 확보. 최소 512px, 권장 1024px+ 정사각.
2. **누끼 엔진** — 기본 **`rembg`(u2net, ML 매팅)** — 임의 배경 대응(`pip install rembg`, 첫 실행 시 모델 ~176MB 다운). 흰/단색 배경이면 §4.8 도구(remove.bg)도 가능. **Pillow floodfill은 단색 배경 한정**(§4.9 belly 1-2% 잔재 전례 — 임의 배경에 쓰지 말 것). 흰 배경엔 `alpha_matting=True`로 fringe 제거.
3. **정규화** — 누끼 알파의 tight bbox 산출 → **긴 변(`max(bbox_w, bbox_h)`) = 캔버스 ~90%**(짧은 변 ~5% safe padding)로 종횡비 **보존** 스케일 → 정사각 1:1 투명 캔버스(1024 마스터, §4.5) 중앙 배치. ⚠️ **짧은 변까지 90%로 강제하지 말 것**(캐릭터 왜곡). 세로로 긴 인물은 좌우 패딩이 자연히 커지는 게 정상(예: 비율 1:2.4 → 폭 ~37%, 높이 ~90%). 복수 캐릭터는 같은 긴 변 90%로 통일 → 시각 무게 일치.
4. **네이밍·경로** (§4.5/§4.6 상속) — 단일 슬롯=`{name}-alpha.png`, 다슬롯=§4.5 사이즈 접미사. 원본 `{name}.png`은 RGB reference로 **보존**(마스코트 `main.png`/`main-alpha.png` 컨벤션 정합). 배치: **2차/보조 캐릭터 → `apps/web/public/brand/`**, 마스코트 → `apps/web/public/mascot/`(§4.6), 아카이브 원본 → `docs/visual-assets/`.
5. **QA 게이트** — §4.7 9종 + 누끼 전용 3종: ① 4모서리 alpha=0(배경 완전 투명) ② 외곽 border-ring 흰-opaque 잔재 ~0px ③ anti-alias 알파 보존(이진화 금지). 육안 합성 확인(체커보드+크림 배경) — 본체 구멍 0, 머리카락 halo 0. (다리 사이·팔-몸통 사이 비침은 정상) **자동 검증**: `python3 scripts/verify-alpha-normalization.py [파일…]` — 정사각/긴 변 90%/대칭 패딩/모서리 alpha=0을 실측해 PASS/FAIL(종료코드). 재현성 보증을 위해 적용 후 반드시 실행.
6. **톤 정합** — 배경 위 렌더 시 `#F8FBF7` 정합(§4.4 4번 룰, 자산 `#F7F4F2` vs 앱 `#F8FBF7` 미세 색차 노출 방지).
...
| **v1.2** | **2026-06-16** | **SOO-45: 디자인 토큰 팔레트를 시안 UI v2(teal/green) 앵커 7종으로 교체(`tokens.css` + `docs/design-tokens.md` v1.1). G2(디자인 토큰 lock) 게이트 재확인 — `pnpm lint:tokens`/`pnpm build` 통과. Ocean(SOO-17)→teal 전환이라 §2 시안 SSoT·CLAUDE.md §2/§8·design-system-lock v1.0 동반 갱신 필요. primary CTA 최종 hex(#5E9C7B vs #4CAF88)는 Mike 확정 대기(placeholder).** |
| **v1.3** | **2026-06-16** | **SOO-45: Mike 확정 맵 반영(`design-tokens.md` v1.2). primary CTA fill = `#5E9C7B` 확정, `--color-primary-bg` 신규, text 4단 중성 그레이 확정, bg-soft/border-soft/state bg 확정값. `pnpm lint:tokens`(32 토큰)·`pnpm build` 통과. Ocean→teal 잠금 문서(CLAUDE.md §2/§8·design-system-lock v1.0) 동반 갱신은 후속.** |
