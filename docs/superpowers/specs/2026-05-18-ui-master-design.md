...
3. **정규화** — 누끼 알파의 tight bbox 산출 → 캐릭터 bbox = 캔버스 **~90%**(상하좌우 ~5% safe padding)로 스케일 → 정사각 1:1 투명 캔버스(1024 마스터, §4.5) 중앙 배치. 복수 캐릭터는 같은 90%로 통일 → 시각 무게 일치.
...
5. **QA 게이트** — §4.7 9종 + 누끼 전용 3종: ① 4모서리 alpha=0(배경 완전 투명) ② 외곽 border-ring 흰-opaque 잔재 ~0px ③ anti-alias 알파 보존(이진화 금지). 육안 합성 확인(체커보드+크림 배경) — 본체 구멍 0, 머리카락 halo 0. (다리 사이·팔-몸통 사이 비침은 정상)
...
**1차 적용 (SOO-79, 2026-06-19 완료):** `sub-boy` / `sub-girl`(휴먼 보조, 3D 트랙) 흰 배경 rembg 누끼 → `apps/web/public/brand/sub-{boy,girl}-alpha.png`(1024² RGBA, bbox 90% 정규화). QA: 4모서리 alpha=0, border 잔재 0px 통과.
